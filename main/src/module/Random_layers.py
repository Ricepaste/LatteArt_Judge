...
class SETSparseLayer(nn.Module):
    ...
    def update_topology(self, grow_ratio=0.2):
        with torch.no_grad():
            ...
            # =======================================================
            # 1. Prune (保持不變：Drop 最小權重 Magnitude)
            # =======================================================
            active_weights = torch.abs(self.layer.weight) * self.mask
            # 將死掉的權重設為無限大，確保不會被選中為"最小"
            ...
            # =======================================================
            # 2. Grow (改為：純隨機 Random Growth)
            # =======================================================
            # 生成與權重形狀相同的隨機分數 (0~1)
            random_scores = torch.rand_like(self.layer.weight)
            
            # 只在 Mask=0 (空連結) 的區域進行隨機選取
            candidate_scores = random_scores * (1 - self.mask)
            
            # 選出隨機分數最高的 num_swap 個位置
            ...
            # 新生長的權重初始化為 0
            self.layer.weight.data[grow_mask] = 0.0 
            
            # 重置 Hebbian 分數 (雖然隨機生長不用這個，但保持狀態乾淨)
    ...
    def _convert_to_hebbian(self, module, sparsity):
        ...
        for name, child in module.named_children():
            if isinstance(child, nn.Conv2d):
                new_module.add_module(name, SETSparseLayer(child, sparsity))
            else:
